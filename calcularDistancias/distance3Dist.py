...
capacidade = 500
# As três primeiras posições são distribuidoras e têm demanda 0
demandas = [0, 0, 0, 120, 170, 145, 150, 160, 70, 80, 80, 50, 140, 90, 120, 150, 60, 95, 60, 90, 140, 145, 170, 80, 150, 60, 145, 160, 65, 55, 90, 100, 95] 
...
def gerar_rota_sequencial(locais, demandas, capacidade):
    ...
    cumul = 0
    for i in range(3, len(locais)):
        if cumul + demandas[i] > capacidade:
            distancias = [matriz_distancias[i][j] for j in range(3)]
...
# Se quiser calcular para a rota sequencial:
...
print("")
precoTotal = (distancia_sequencial / 2500) * 6.44 # Calculo do custo por uma viagem inteira
print(f"Preço total pela viagem: R${precoTotal:.2f}")
rota_sequencial = gerar_rota_sequencial(locais, demandas, capacidade)
print("")
